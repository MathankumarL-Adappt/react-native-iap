...
public class RNIapModule extends ReactContextBaseJavaModule implements PurchasesUpdatedListener {
    ...
    @ReactMethod
    public void validateLicence(final Promise promise) {
        ...
        mLicenseCheckerCallback = new MyLicenseCheckerCallback(promise);
        // Construct the LicenseChecker with a policy.

        mChecker = new LicenseChecker(
                reactContext, new ServerManagedPolicy(reactContext,
                new AESObfuscator(SALT, reactContext.getPackageName(), deviceId)),
                BASE64_PUBLIC_KEY);
    ...
    private class MyLicenseCheckerCallback implements LicenseCheckerCallback {
        ...
        public MyLicenseCheckerCallback(Promise promise) {
            this.promise = promise;

        }

        public void allow(int policyReason, ResponseData responseData) {

            try {
                WritableMap licenseResponse = Arguments.createMap();
                licenseResponse.putInt("policyReason", policyReason);
                if (responseData != null) {
                    licenseResponse.putString("license_data", responseData.toString());
                }
                licenseResponse.putString("Status", "Sucess");
                licenseResponse.putString("signature", responseData.getSignature());
                licenseResponse.putString("signedData", responseData.getSignedData());
                promise.resolve(licenseResponse);


            } catch (Exception e) {
                promise.resolve("Success but error while parsing data " + e.getMessage());
            }
        ...
        public void dontAllow(int policyReason) {
            ...
            try {
                ...
                licenseResponse.putString("Status", "Failure");
                promise.resolve(licenseResponse);
            } catch (Exception e) {
                promise.resolve("failure  error while parsing data " + e.getMessage());
            }
        ...
        public void applicationError(int errorCode) {
            ...
            try {
                ...
                item.putInt("errorCode", errorCode);
                item.putString("Status", "Application error");
                promise.resolve(item);
            } catch (Exception e) {
                promise.resolve("application error" + e.getMessage());
            }
